...
type ProfileDraft={username:string;profession:string;headline:string;industry:string;bio:string;primarySkill:string;secondarySkill:string;tertiarySkill:string;interests:string;location:string;workMode:"remote"|"hybrid"|"in_person"};
...
const initialDraft:ProfileDraft={username:"",profession:"",headline:"",industry:"",bio:"",primarySkill:"",secondarySkill:"",tertiarySkill:"",interests:"",location:"",workMode:"hybrid"};

function identityError(draft:ProfileDraft,safeguardsEnabled:boolean){
  const profession=draft.profession.trim(),industry=draft.industry.trim(),bio=draft.bio.trim();
  if(!/^[a-z0-9][a-z0-9_-]{2,29}$/.test(draft.username))return "Choose a username using 3–30 lowercase letters, numbers, underscores or hyphens.";
  ...
  if(safeguardsEnabled&&!canonicalProfession(profession))return "Choose a profession from the list.";
  if(safeguardsEnabled&&canonicalProfession(profession)===OTHER_PROFESSION&&!isMeaningfulOtherHeadline(draft.headline))return "Describe your unlisted profession using at least two meaningful words.";
  ...
  if(safeguardsEnabled&&!canonicalIndustry(industry))return "Choose an industry from the list.";
...
export default function OnboardingPage(){
  ...
  useEffect(()=>{fetch("/api/auth/onboarding",{cache:"no-store"}).then(response=>response.ok?response.json():null).then(result=>{if(!result)return;if(result.username)setDraft(current=>({...current,username:result.username}));if(typeof result.profileTaxonomySafeguardsEnabled==="boolean")setSafeguardsEnabled(result.profileTaxonomySafeguardsEnabled)}).catch(()=>undefined)},[]);
  ...
  async function next(){
    setError("");
    const message=step===0?identityError(draft,safeguardsEnabled):step===1?skillsError(draft):"";
    if(message){setError(message);return}
    if(step===0){
      setBusy(true);
      try{
        const response=await fetch(`/api/auth/onboarding?username=${encodeURIComponent(draft.username)}`,{cache:"no-store"}),result=await response.json().catch(()=>({}));
        if(!response.ok){setError(result.error??"We could not check that username. Try again.");return}
        if(!result.available){setError(result.reason==="format"?"Choose a valid username using lowercase letters, numbers, underscores or hyphens.":"That username is already taken. Choose another one.");return}
      }catch{setError("We could not check that username. Try again.");return}finally{setBusy(false)}
    }
  ...
  return <main className="onboarding-page onboarding-flow"><div className="onboarding-backdrop"/><section className="onboarding-modal" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">
    ...
    <div className="onboarding-viewport"><div className="onboarding-track" style={{transform:`translateX(-${step*100}%)`}}>
      <section className="onboarding-slide" aria-hidden={step!==0} inert={step!==0}><div className="onboarding-copy"><span className="verified-banner compact"><Check size={14}/><span><strong>Account created</strong><small>Let’s start building your community.</small></span></span><span className="eyebrow">PROFESSIONAL IDENTITY</span><h1 id="onboarding-title">What should people know about you?</h1><p>This shapes how members and projects understand the contribution you can make. Use your genuine professional details so discovery and recommendations stay useful.</p></div><div className="onboarding-fields"><label className="full">Username<span className="onboarding-username"><b aria-hidden="true">@</b><input value={draft.username} onChange={event=>update("username",event.target.value.toLowerCase().replace(/[^a-z0-9_-]/g,""))} placeholder="your-name" autoCapitalize="none" autoCorrect="off" spellCheck={false} minLength={3} maxLength={30} aria-describedby="onboarding-username-help"/></span><small id="onboarding-username-help">Your profile address will be /{draft.username||"your-name"}. You can choose public visibility later in Settings.</small></label><label htmlFor="onboarding-profession">Profession{ safeguardsEnabled?<CareerIndustryInput id="onboarding-profession" mode="profession" strict value={draft.profession} onChange={value=>update("profession",value)} placeholder="Search professions" required/>:<input id="onboarding-profession" value={draft.profession} onChange={event=>update("profession",event.target.value)} placeholder="e.g. Product designer" minLength={2} maxLength={100}/>}</label><label htmlFor="onboarding-industry">Industry<CareerIndustryInput id="onboarding-industry" value={draft.industry} onChange={value=>update("industry",value)} placeholder="Type an industry or career" required strict={safeguardsEnabled} ariaDescribedBy="onboarding-industry-help"/><small id="onboarding-industry-help">Search by career to find its sector.</small></label>{safeguardsEnabled&&draft.profession===OTHER_PROFESSION&&<label className="full">Professional headline<input value={draft.headline} onChange={event=>update("headline",event.target.value)} placeholder="Describe your unlisted profession" minLength={6} maxLength={160}/><small>Use at least two words to describe your genuine professional role.</small></label>}<SuggestionChips label="Suggested roles" options={PROFESSION_SUGGESTIONS} selected={[draft.profession]} onSelect={value=>update("profession",value)}/><SuggestionChips label="Suggested industries" options={INDUSTRY_SUGGESTIONS} selected={[draft.industry]} onSelect={value=>update("industry",value)}/><label className="full">Short bio<textarea value={draft.bio} onChange={event=>update("bio",event.target.value)} placeholder="Describe your experience, the work you enjoy and what you can contribute…" minLength={ONBOARDING_BIO_MIN_LENGTH} maxLength={600} aria-describedby="short-bio-hint"/><small id="short-bio-hint">At least 6 words and {ONBOARDING_BIO_MIN_LENGTH} characters · {bioWordCount(draft.bio)} words · {draft.bio.length}/600 used</small></label></div><SlideFooter step={step} error={error} busy={busy} busyLabel="Checking username…" onBack={()=>{}} onNext={next}/></section>
      <section className="onboarding-slide" aria-hidden={step!==1} inert={step!==1}><div className="onboarding-copy"><span className="eyebrow">CAREER SKILLS</span><h1>Rank the skills you want to be known for.</h1><p>Choose three different skills. Tap a field, then use a suggestion or enter your own.</p></div><div className="onboarding-skill-fields"><label><b>1</b><span>Primary skill<small>Your strongest career skill.</small></span><input value={draft.primarySkill} onFocus={()=>setActiveSkill("primarySkill")} onChange={event=>update("primarySkill",event.target.value)} placeholder="Product strategy" maxLength={80}/></label><label><b>2</b><span>Secondary skill<small>A strong supporting capability.</small></span><input value={draft.secondarySkill} onFocus={()=>setActiveSkill("secondarySkill")} onChange={event=>update("secondarySkill",event.target.value)} placeholder="User research" maxLength={80}/></label><label><b>3</b><span>Tertiary skill<small>Another skill you would like to share.</small></span><input value={draft.tertiarySkill} onFocus={()=>setActiveSkill("tertiarySkill")} onChange={event=>update("tertiarySkill",event.target.value)} placeholder="Prototyping" maxLength={80}/></label><SuggestionChips label={`Suggestions for ${activeSkill==="primarySkill"?"primary":activeSkill==="secondarySkill"?"secondary":"tertiary"} skill`} options={SKILL_SUGGESTIONS} selected={skillFields.map(field=>draft[field])} onSelect={chooseSkill}/></div><SlideFooter step={step} error={error} onBack={()=>setStep(0)} onNext={next}/></section>
      <section className="onboarding-slide" aria-hidden={step!==2} inert={step!==2}><div><div className="onboarding-copy"><span className="eyebrow">INTERESTS & AVAILABILITY</span><h1>What kind of work should find you?</h1><p>Choose at least two topics so your first suggestions have enough signal.</p></div><div className="onboarding-fields"><label className="full">Interests<input value={draft.interests} onChange={event=>update("interests",event.target.value)} placeholder="Climate action, local communities, public good"/><small>Choose or enter at least two · separate your own interests with commas.</small></label><SuggestionChips label="Suggested interests" options={INTEREST_SUGGESTIONS} selected={parseOnboardingInterests(draft.interests)} onSelect={toggleInterest}/><label>Location<span className="onboarding-location"><MapPin size={15}/><input value={draft.location} onChange={event=>update("location",event.target.value)} placeholder="London, UK" minLength={2} maxLength={100}/></span></label><label>Preferred working style<N2Select value={draft.workMode} onValueChange={value=>update("workMode",value)} ariaLabel="Preferred working style" options={[{value:"remote",label:"Remote"},{value:"hybrid",label:"Hybrid"},{value:"in_person",label:"In person"}]}/></label></div><SlideFooter step={step} error={error} onBack={()=>setStep(1)} onNext={()=>{const message=interestsError(draft);if(message){setError(message);return}setError("");setStep(3)}}/></div></section>
      <section className="onboarding-slide" aria-hidden={step!==3} inert={step!==3}><form onSubmit={submit}><div className="onboarding-copy"><span className="eyebrow">NETWORK VISIBILITY</span><h1>Open the door to new connections.</h1><p>Help people discover shared interests, ideas and opportunities. We only show people who allow it, and every introduction is up to you.</p></div><div className="onboarding-visibility"><button type="button" className={networking.shareNetworkConnections?"active":""} aria-pressed={networking.shareNetworkConnections} onClick={()=>setNetworking(value=>({...value,shareNetworkConnections:!value.shareNetworkConnections}))}><UsersRound size={20}/><span><strong>Help people discover my network</strong><small>Show approved connections to people who follow you.</small></span><i>{networking.shareNetworkConnections?"On":"Off"}</i></button><button type="button" className={networking.allowIntroductions?"active":""} aria-pressed={networking.allowIntroductions} onClick={()=>setNetworking(value=>({...value,allowIntroductions:!value.allowIntroductions}))}><Sparkles size={20}/><span><strong>Let people ask for an introduction</strong><small>You choose which requests to accept—nothing happens without your say-so.</small></span><i>{networking.allowIntroductions?"On":"Off"}</i></button><p>You’re always in control. Change these choices any time in Settings → Networking.</p></div><SlideFooter step={step} error={error} busy={busy} onBack={()=>setStep(2)} submit nextLabel="Build my network"/></form></section>
      <section className="onboarding-slide suggestion-slide" aria-hidden={step!==4} inert={step!==4}><div className="onboarding-copy"><span className="eyebrow">PEOPLE FOR YOUR NETWORK</span><h1>Start connecting with like-minded individuals.</h1><p>{network.length?"Say hello, explore shared interests and discover how you can support each other.":"Your profile is ready. New people to connect with will appear as the community grows."}</p></div><div className="people-suggestion-grid">{network.slice(0,4).map(member=><article key={member.id}>{member.image?<img src={member.image} alt=""/>:<span className="suggestion-avatar">n2</span>}<div><strong>{member.name??"n2 member"}</strong><small>{member.profession??"Member"}{member.location?` · ${member.location}`:""}</small><p>{member.reasons.slice(0,2).join(" · ")||"Shared interests and experience"}</p></div><b>{member.score}</b></article>)}{!network.length&&<EmptySuggestion icon="people"/>}</div><SlideFooter step={step} onBack={()=>setStep(3)} onNext={()=>setStep(5)} nextLabel="See project suggestions"/></section>
      <section className="onboarding-slide suggestion-slide project-suggestion-slide" aria-hidden={step!==5} inert={step!==5}><div className="onboarding-copy"><span className="eyebrow">PROJECTS FOR YOU</span><h1>Your first places to contribute.</h1><p>{projects.length?"These active projects align with the profile you just built.":"Your profile is ready. Matching projects will surface as suitable roles open."}</p></div><div className="project-suggestion-grid">{projects.slice(0,4).map(project=><article key={project.id}><span>{project.industry}</span><h3>{project.title}</h3><p>{project.summary}</p><div>{project.reasons.slice(0,2).map(reason=><i key={reason}>{reason}</i>)}</div></article>)}{!projects.length&&<EmptySuggestion icon="projects"/>}</div><footer className="onboarding-final"><button type="button" className="secondary-button" onClick={()=>setStep(4)}><ArrowLeft size={15}/> Back</button><Link className="primary-button" href="/signin">Complete sign up <ArrowRight size={16}/></Link></footer></section>
    </div></div>
  </section></main>;
}

function SlideFooter({step,error,onBack,onNext,nextLabel="Continue",busy=false,busyLabel="Finding your matches…",submit=false}:{step:number;error?:string;onBack:()=>void;onNext?:()=>void;nextLabel?:string;busy?:boolean;busyLabel?:string;submit?:boolean}){return <footer className="onboarding-actions"><span>{error&&<b role="alert">{error}</b>}</span>{step>0&&<button type="button" className="secondary-button" onClick={onBack}><ArrowLeft size={15}/> Back</button>}<button type={submit?"submit":"button"} className="primary-button" onClick={submit?undefined:onNext} disabled={busy}>{busy?<>{busyLabel} <Sparkles size={15}/></>:<>{nextLabel} <ArrowRight size={15}/></>}</button></footer>}
