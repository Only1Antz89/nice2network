...
type ProjectRecord = { id:string; title:string; summary:string; description?:string|null; industry:string; stage:string; status?:string; visibility?:string; accent:string; ownerId?:string; ownerName:string|null; ownerImage:string|null; ownerIsAdmin?:boolean; isOwner?:boolean; isPinned?:boolean; isStarred?:boolean; eyeCount:number; commentCount?:number; createdAt:string };
...
function ProjectsView({ onCreate, latestProject, onComments, onProfile, onShare, onToast }: { onCreate: () => void; latestProject:ProjectRecord|null; onComments:(project:ProjectRecord)=>void; onProfile:(userId:string)=>void; onShare:(project:{id:string;title:string;summary:string})=>void; onToast:(message:string)=>void }) {
  ...
  return (
    <div className="subpage">
      ...
      {latestProject && !records.some(record=>record.id===latestProject.id)&&<ProjectCard project={latestProject} onShare={onShare} onComments={onComments} onProfile={onProfile} onToast={onToast}/>}
      {loading&&<div className="feed-context"><Clock3 size={16}/><span>Loading your project workspaces…</span></div>}
      {records.map(record=><ProjectCard key={record.id} project={record} onShare={onShare} onComments={onComments} onProfile={onProfile} onToast={onToast} onChanged={next=>setRecords(rows=>next?rows.map(row=>row.id===next.id?next:row):rows.filter(row=>row.id!==record.id))}/>)}
      <ProjectWorkbench />
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="content-column">
          ...
          {view==="projects"&&<ProjectsView onCreate={()=>setCreateOpen(true)} latestProject={latestProject} onComments={setCommentProject} onProfile={openProfile} onShare={setShareProject} onToast={setToast}/>}
