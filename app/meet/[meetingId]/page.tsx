/* eslint-disable no-empty, @next/next/no-html-link-for-pages, jsx-a11y/label-has-associated-control, jsx-a11y/media-has-caption, react-hooks/refs */
"use client";
...
function DeviceSettings({ devices, microphoneId, cameraId, speakerId, video, qualityMode, effectiveQuality, qualitySnapshot, onQuality, onClose, onInput, onSpeaker }: { devices: { microphones: DeviceChoice[]; cameras: DeviceChoice[]; speakers: DeviceChoice[] }; microphoneId: string; cameraId: string; speakerId: string; video: boolean; qualityMode?: VideoQualityMode; effectiveQuality?: VideoQualityPreset; qualitySnapshot?: QualitySnapshot; onQuality?: (quality: VideoQualityMode) => void; onClose: () => void; onInput: (kind: "audio" | "video", id: string) => void; onSpeaker: (id: string) => void }) {
  const deviceOptions = (choices: DeviceChoice[]) => choices.map(device => ({ value: device.deviceId, label: device.label }));
  return <div className="room-settings-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && onClose()}><section className="room-settings"><header><div><span>YOUR DEVICES</span><h2>Audio & video settings</h2></div><button onClick={onClose}><X/></button></header><label><Mic/>Microphone<N2Select value={microphoneId} onValueChange={value => onInput("audio", value)} ariaLabel="Microphone" options={deviceOptions(devices.microphones)}/></label>{video && <><label><Camera/>Camera<N2Select value={cameraId} onValueChange={value => onInput("video", value)} ariaLabel="Camera" options={deviceOptions(devices.cameras)}/></label><label><Video/>Video quality<N2Select value={qualityMode} onValueChange={value => onQuality?.(value as VideoQualityMode)} ariaLabel="Video quality" options={[{value:"auto",label:"Auto (recommended)"},{value:"high",label:"High · up to 1080p"},{value:"standard",label:"Standard · up to 720p"},{value:"data",label:"Data saver · up to 360p"}]}/></label><div className="quality-settings-summary"><span className={`quality-dot quality-${qualitySnapshot?.level ?? "checking"}`}/><div><strong>{qualityMode === "auto" ? `Auto is using ${effectiveQuality}` : `${effectiveQuality} quality`}</strong><small>{qualitySnapshot?.width && qualitySnapshot?.height ? `${qualitySnapshot.width}×${qualitySnapshot.height}` : "Measuring video"}{qualitySnapshot?.fps ? ` · ${Math.round(qualitySnapshot.fps)} fps` : ""}{qualitySnapshot?.roundTripMs ? ` · ${qualitySnapshot.roundTripMs} ms` : ""}{qualitySnapshot?.availableKbps ? ` · ${qualitySnapshot.availableKbps} kbps` : ""}</small></div></div></>}<label><Volume2/>Speakers<N2Select value={speakerId} onValueChange={onSpeaker} ariaLabel="Speakers" disabled={!devices.speakers.length} options={devices.speakers.length ? deviceOptions(devices.speakers) : [{value:"",label:"System default"}]}/></label><button className="primary-action" onClick={onClose}><Check/>Done</button></section></div>;
}
