...
import { adminAssignments, projectComments, projects, users } from "@/db/schema";
...
export async function GET(_: Request, { params }: { params: Promise<{ projectId: string }> }) {
  try {
    await requireMember(); const { projectId } = await params, db = getDb();
    const [project] = await db.select({ id: projects.id, visibility: projects.visibility }).from(projects).where(eq(projects.id, projectId)).limit(1);
