...
export async function GET(_: Request, { params }: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    if (member?.id) await requireProjectView(member.id, projectId);
    ...
    if (!project || (!member?.id && project.visibility !== "network") || (project.visibility === "private" && (!member?.id||(project.ownerId !== member.id && !membership)))) throw new ApiError(404, "Project not found");
