...
const schema = z.object({ roles: z.array(blueprintRoleSchema).min(1).max(18), milestones:z.array(milestoneSchema).min(1).max(15), visibility: z.enum(["network", "connections", "private"]).default("network"), coOwnerIds: coOwnerIdsSchema });
...
export async function POST(request: Request, { params }: { params: Promise<{ projectId: string; blueprintId: string }> }) {
  try {
    ...
    const result = await approveProjectBlueprint({ projectId, blueprintId, userId: member.id, roles: input.roles, milestones:input.milestones, visibility: input.visibility, coOwnerIds: input.coOwnerIds });
