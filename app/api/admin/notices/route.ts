...
export async function POST(request:Request){try{const admin=await requirePermission("notices.manage"),db=getDb();const input=schema.parse(await request.json());const [notice]=await db.insert(officialNotices).values({...input,authorId:admin.user.id,expiresAt:input.expiresAt?new Date(input.expiresAt):null}).returning();const audience=await db.select({id:users.id}).from(users).where(eq(users.status,"active"));await createNotifications(audience.map(member=>({userId:member.id,actorId:admin.user.id,type:"official" as const,title:input.title,body:input.body.slice(0,180),entityType:"notice",entityId:notice.id,href:"/"})));await audit(admin.user.id,"admin.official_notice_published","notice",notice.id,{audience:input.audience},{permission:"notices.manage",reason:input.title,severity:"info"});return NextResponse.json(notice,{status:201})}catch(error){return apiError(error)}}
