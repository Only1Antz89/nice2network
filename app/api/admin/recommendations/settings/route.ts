...
import { createProjectEmbeddingReindexJob } from "@/lib/recommendations/project-similarity";
...
const schema = z.object({ provider: z.enum(["openai", "gemini"]), blueprintModel: z.string().trim().min(2).max(100), embeddingModel: z.string().trim().min(2).max(100), rolloutStage: z.number().int().min(1).max(3), similarProjectSuggestionsEnabled: z.boolean().default(true), weights: weightsSchema.default(DEFAULT_ALGORITHM_WEIGHTS), reason: z.string().trim().min(10).max(500) }).superRefine((value, ctx) => {
...
export async function POST(request: Request) {
  try {
    ...
    const [settings] = await db.transaction(async tx => {
      ...
      return tx.insert(algorithmSettings).values({ version: nextVersion, status: "active", provider: input.provider, blueprintModel: input.blueprintModel, embeddingModel: input.embeddingModel, rolloutStage: input.rolloutStage, similarProjectSuggestionsEnabled: input.similarProjectSuggestionsEnabled, weights: input.weights, createdBy: admin.user.id, activatedAt: new Date() }).returning();
    });
    let reindexJob = null, projectReindexJob = null;
    if (before.provider !== input.provider || before.embeddingModel !== input.embeddingModel) {
      [reindexJob, projectReindexJob] = await Promise.all([createEmbeddingReindexJob(admin.user.id, input.provider), createProjectEmbeddingReindexJob(admin.user.id, input.provider)]);
    }
    await audit(admin.user.id, "recommendation.settings_activated", "algorithm_settings", settings.id, { reindexJobId: reindexJob?.id, projectReindexJobId: projectReindexJob?.id }, { permission: "system.manage", reason: input.reason, before, after: { provider: settings.provider, blueprintModel: settings.blueprintModel, embeddingModel: settings.embeddingModel, rolloutStage: settings.rolloutStage, similarProjectSuggestionsEnabled: settings.similarProjectSuggestionsEnabled, version: settings.version } });
    return NextResponse.json({ settings, reindexJob, projectReindexJob });
  } catch (error) { return apiError(error); }
}
