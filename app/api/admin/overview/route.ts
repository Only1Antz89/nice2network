...
import { applications, appeals, integrationAccounts, meetings, productEvents, projectRoles, projects, reports, safetyRisks, sanctions, users } from "@/db/schema";
...
export async function GET() {
  try {
    ...
    const [memberCount, pendingCount, activeProjectCount, openRoleCount, openReportCount, urgentRiskCount, upcomingMeetingCount, applicationCount, appealCount, sanctionCount, eventCount, integrations] = await Promise.all([
    ...
    return NextResponse.json({ metrics: { activeMembers: memberCount[0].value, pendingMembers: pendingCount[0].value, activeProjects: activeProjectCount[0].value, openRoles: openRoleCount[0].value, openReports: openReportCount[0].value, urgentRisks: urgentRiskCount[0].value, upcomingMeets: upcomingMeetingCount[0].value, pendingApplications: applicationCount[0].value, openAppeals: appealCount[0].value, activeSanctions: sanctionCount[0].value, productEvents30d: eventCount[0].value }, integrations, generatedAt: new Date() });
