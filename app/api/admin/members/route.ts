...
export async function GET(request: Request) {
  try {
    ...
    const rows = await getDb().select({ id: users.id, name: users.name, email: users.email, emailVerified:users.emailVerified, onboardingCompletedAt:users.onboardingCompletedAt, image: users.image, profession: users.profession, status: users.status, ageBand: users.ageBand, createdAt: users.createdAt, adminRole: adminAssignments.role, adminStatus: adminAssignments.status }).from(users).leftJoin(adminAssignments, eq(adminAssignments.userId, users.id)).where(query ? or(ilike(users.name, `%${query}%`), ilike(users.email, `%${query}%`), ilike(users.profession, `%${query}%`)) : undefined).orderBy(desc(users.createdAt)).limit(100);
