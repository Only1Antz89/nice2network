...
export async function GET(request: Request) {
  try {
    ...
    const connectionsOnly = searchParams.get("scope") === "connections";
    if (query.length < 2 && !connectionsOnly) return NextResponse.json({ people: [], projects: [], roles: [] });
    ...
    const [people, projectRows, roles] = await Promise.all([
      db.select({ id: users.id, username: users.username, name: users.name, image: users.image, profession: users.profession, industry: users.industry, primarySkill: users.primarySkill, secondarySkill: users.secondarySkill, tertiarySkill: users.tertiarySkill, skills: users.skills, interests: users.interests, messagePermission: privacySettings.messagePermission, isFollowing, followsViewer, sharesProject, isN2Admin: sql<boolean>`case when ${adminAssignments.status} = 'active' then true else false end`, isDemo: sql<boolean>`${users.role} = 'demo_member'` })
        ...
        .where(and(ne(users.id, member.id), eq(users.status, "active"), inArray(users.ageBand, discoverableAgeBands), or(eq(privacySettings.profileVisibility, "network"), sql`${privacySettings.userId} is null`), connectionsOnly ? and(isFollowing, followsViewer) : undefined, or(ilike(users.username, term), ilike(users.name, term), ilike(users.profession, term), ilike(users.industry, term), ilike(users.primarySkill, term), ilike(users.secondarySkill, term), ilike(users.tertiarySkill, term), sql`array_to_string(${users.skills}, ' ') ilike ${term}`, sql`array_to_string(${users.interests}, ' ') ilike ${term}`))).orderBy(sql`${isFollowing} desc`,sql`(${isFollowing} and ${followsViewer}) desc`,sql`${sharesProject} desc`,users.name).limit(8),
      connectionsOnly ? Promise.resolve([]) : db.select({ id: projects.id, title: projects.title, summary: projects.summary, industry: projects.industry, stage: projects.stage, accent: projects.accent, ownerName: users.name, createdAt: projects.createdAt, isDemo: sql<boolean>`${users.role} = 'demo_member'` })
      ...
      connectionsOnly ? Promise.resolve([]) : db.select({ id: projectRoles.id, projectId: projectRoles.projectId, title: projectRoles.title, department: projectRoles.department, skills: projectRoles.skills, projectTitle: projects.title })
