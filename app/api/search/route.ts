...
export async function GET(request: Request) {
  try {
    ...
    const [people, projectRows, roles] = await Promise.all([
      db.select({ id: users.id, name: users.name, image: users.image, profession: users.profession, industry: users.industry, primarySkill: users.primarySkill, secondarySkill: users.secondarySkill, tertiarySkill: users.tertiarySkill, skills: users.skills, interests: users.interests, messagePermission: privacySettings.messagePermission, isFollowing, followsViewer, sharesProject, isN2Admin: sql<boolean>`case when ${adminAssignments.status} = 'active' then true else false end`, isDemo: sql<boolean>`${users.role} = 'demo_member'` })
        ...
        .where(and(ne(users.id, member.id), eq(users.status, "active"), inArray(users.ageBand, discoverableAgeBands), or(eq(privacySettings.profileVisibility, "network"), sql`${privacySettings.userId} is null`), or(ilike(users.name, term), ilike(users.profession, term), ilike(users.industry, term), ilike(users.primarySkill, term), ilike(users.secondarySkill, term), ilike(users.tertiarySkill, term), sql`array_to_string(${users.skills}, ' ') ilike ${term}`, sql`array_to_string(${users.interests}, ' ') ilike ${term}`))).orderBy(sql`${isFollowing} desc`,sql`(${isFollowing} and ${followsViewer}) desc`,sql`${sharesProject} desc`,users.name).limit(8),
