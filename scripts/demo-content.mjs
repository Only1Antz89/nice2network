...
const people = [
  ['d2000000-0000-4000-8000-000000000001','Maya Chen','maya','Product Designer','Design & public services','Product design','User research','Service design',['Climate action','Public services','Accessible technology'],'London','United Kingdom','hybrid','Designing useful services with communities, not merely for them.','https://i.pravatar.cc/240?img=47'],
  ['d2000000-0000-4000-8000-000000000002','Marcus Okafor','marcus','Clean Energy Founder','Climate & energy','Energy strategy','Community ownership','Financial modelling',['Clean energy','Local resilience','Cooperatives'],'London','United Kingdom','hybrid','Building practical routes to shared, affordable neighbourhood energy.','https://i.pravatar.cc/240?img=12'],
  ['d2000000-0000-4000-8000-000000000003','Lena Vogt','lena','Brand Strategist','Creative industries','Brand strategy','Community launch','Storytelling',['Purposeful brands','Climate','Public good'],'London','United Kingdom','hybrid','Helping early ideas become clear stories people want to join.','https://i.pravatar.cc/240?img=32'],
  ['d2000000-0000-4000-8000-000000000004','Dev Shah','dev','Full-stack Engineer','Technology','Software development','React','Cloud infrastructure',['Open source','Climate tech','Civic technology'],'Manchester','United Kingdom','remote','Full-stack engineer focused on dependable tools for small, ambitious teams.','https://i.pravatar.cc/240?img=11'],
  ['d2000000-0000-4000-8000-000000000005','Ali Rahman','ali','Operations Lead','Community & nonprofit','Operations','Partnerships','Project management',['Social enterprise','Food systems','Neighbourhoods'],'London','United Kingdom','hybrid','Turning promising community concepts into pilots that actually run.','https://i.pravatar.cc/240?img=51'],
  ['d2000000-0000-4000-8000-000000000006','Sofia Reyes','sofia','Urban Planner','Design & public services','Urban planning','Community consultation','Place strategy',['Night-time economy','Public space','Culture'],'London','United Kingdom','in_person','Exploring how overlooked places can become safer, more useful and more social.','https://i.pravatar.cc/240?img=45'],
  ['d2000000-0000-4000-8000-000000000007','Jordan Lee','jordan','Community Builder','Community & nonprofit','Community building','Facilitation','Volunteer programmes',['Youth opportunity','Arts','Local networks'],'Bristol','United Kingdom','hybrid','Creating welcoming networks where people can contribute something real.','https://i.pravatar.cc/240?img=14'],
  ['d2000000-0000-4000-8000-000000000008','Amara Bennett','amara','Finance & Growth Adviser','Finance','Financial planning','Fundraising','Growth strategy',['Startups','Circular economy','Founder support'],'London','United Kingdom','remote','Helping purpose-led founders build credible numbers and sustainable routes to growth.','https://i.pravatar.cc/240?img=5'],
  ['d2000000-0000-4000-8000-000000000009','Noah Williams','noah','Mobile Engineer','Technology','Mobile development','React Native','API development',['Weather','Accessibility','Open data'],'Manchester','United Kingdom','remote','Mobile engineer who enjoys turning public data into calm, reliable products.','https://i.pravatar.cc/240?img=13'],
  ['d2000000-0000-4000-8000-000000000010','Priya Nair','priya','Food Systems Researcher','Food & hospitality','Food systems','Research','Impact measurement',['Food waste','Circular economy','Community kitchens'],'Birmingham','United Kingdom','hybrid','Researching practical ways for cities to waste less food and share more value.','https://i.pravatar.cc/240?img=49'],
  ['d2000000-0000-4000-8000-000000000011','Elena Moore','elena','Materials Scientist','Manufacturing','Materials science','Circular design','Supply chains',['Repair','Circular materials','Local manufacturing'],'Leeds','United Kingdom','hybrid','Turning material research into practical circular products and supply chains.','https://i.pravatar.cc/240?img=44'],
  ['d2000000-0000-4000-8000-000000000012','Kwame Boateng','kwame','Documentary Producer','Creative industries','Documentary production','Story development','Community interviewing',['Local history','Culture','Public storytelling'],'London','United Kingdom','hybrid','Producing careful stories that preserve local knowledge and bring new audiences into it.','https://i.pravatar.cc/240?img=3'],
...
async function seed() {
  await sql.begin(async tx => {
    ...
    for (let pIndex=0;pIndex<projects.length;pIndex++) {
      ...
      await tx`delete from project_members where project_id=${p.id} and membership_role<>'owner'`;
      for (let rIndex=0;rIndex<p.roles.length;rIndex++) {
        ...
        const memberIndex=projectTeams[pIndex][rIndex],filled=memberIndex===undefined?0:1,status=filled?'closed':'open';
        await tx`insert into project_roles (id,project_id,title,department,description,skills,professions,required_skills,useful_skills,phase,criticality,work_mode,reason,capacity,filled,status)
          values (${roleId},${p.id},${title},${department},${`Help ${p.title} reach its next milestone.`},${skills},${[title]},${skills},${[]},${rIndex===0?'now':rIndex===1?'next':'later'},${rIndex===0?'critical':'important'},${p.mode},${`This contribution closes a practical gap in ${p.title}.`},1,${filled},${status}) on conflict (id) do update set title=excluded.title,department=excluded.department,skills=excluded.skills,professions=excluded.professions,required_skills=excluded.required_skills,phase=excluded.phase,capacity=excluded.capacity,filled=excluded.filled,status=excluded.status`;
        if(memberIndex!==undefined){const member=people[memberIndex];await tx`insert into project_members (project_id,user_id,role_id,membership_role,department) values (${p.id},${member[0]},${roleId},'contributor',${title}) on conflict (project_id,user_id) do update set role_id=excluded.role_id,membership_role='contributor',department=excluded.department`}
      }
