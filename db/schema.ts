...
export const projects = pgTable("projects", {
  id: uuid("id").defaultRandom().primaryKey(), ownerId: uuid("owner_id").notNull().references(() => users.id), title: text("title").notNull(), summary: text("summary").notNull(), description: text("description"), imageUrl: text("image_url"), industry: text("industry").notNull(), stage: text("stage").notNull().default("idea"), status: text("status").notNull().default("active"), visibility: text("visibility").notNull().default("network"), location: text("location"), city: text("city"), country: text("country"), timezone: text("timezone").notNull().default("Europe/London"), workMode: text("work_mode").notNull().default("remote"), allowRemoteFallback: boolean("allow_remote_fallback").notNull().default(false), accent: text("accent").notNull().default("#ff6b35"), fundingGoal: integer("funding_goal"), shareLimit: integer("share_limit"), openToInvestment: boolean("open_to_investment").notNull().default(false), openToContributions: boolean("open_to_contributions").notNull().default(true), completedAt: timestamp("completed_at", { withTimezone: true }), deletionPreviousStatus: text("deletion_previous_status"), deletionRequestedAt: timestamp("deletion_requested_at", { withTimezone: true }), deletionScheduledAt: timestamp("deletion_scheduled_at", { withTimezone: true }), deletionRequestedBy: uuid("deletion_requested_by").references(() => users.id), deletedAt: timestamp("deleted_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(), updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("projects_owner_idx").on(table.ownerId), index("projects_status_idx").on(table.status), index("projects_deletion_schedule_idx").on(table.status, table.deletionScheduledAt)]);

export const contentDrafts = pgTable("content_drafts", {
  id: uuid("id").defaultRandom().primaryKey(),
  ownerId: uuid("owner_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  kind: text("kind").$type<"project" | "post">().notNull(),
  title: text("title"),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("content_drafts_owner_kind_idx").on(table.ownerId, table.kind, table.updatedAt), uniqueIndex("content_drafts_project_unique").on(table.projectId)]);
...
export const projectFundingInterests = pgTable("project_funding_interests", {
  id: uuid("id").defaultRandom().primaryKey(),
  projectId: uuid("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("project_funding_interest_project_idx").on(table.projectId, table.createdAt)]);
...
export const invitations = pgTable("invitations", {
  id: uuid("id").defaultRandom().primaryKey(), projectId: uuid("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }), roleId: uuid("role_id").references(() => projectRoles.id), invitedBy: uuid("invited_by").notNull().references(() => users.id), inviteeId: uuid("invitee_id").references(() => users.id), email: text("email"), membershipRole: text("membership_role").notNull().default("contributor"), tokenHash: text("token_hash").notNull().unique(), status: text("status").notNull().default("pending"), expiresAt: timestamp("expires_at", { withTimezone: true }).notNull(), respondedAt: timestamp("responded_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("invitations_project_role_status_idx").on(table.projectId, table.membershipRole, table.status, table.expiresAt)]);

export const milestones = pgTable("milestones", {
  id: uuid("id").defaultRandom().primaryKey(), projectId: uuid("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }), title: text("title").notNull(), description: text("description"), phase: text("phase").notNull().default("now"), ownerId: uuid("owner_id").references(() => users.id), status: text("status").notNull().default("planned"), dueAt: timestamp("due_at", { withTimezone: true }), startedAt: timestamp("started_at", { withTimezone: true }), completedAt: timestamp("completed_at", { withTimezone: true }), completionSummary: text("completion_summary"), sortOrder: integer("sort_order").notNull().default(0), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("milestones_project_idx").on(table.projectId)]);

export const projectUpdates = pgTable("project_updates", {
  id: uuid("id").defaultRandom().primaryKey(), projectId: uuid("project_id").notNull().references(() => projects.id, { onDelete: "cascade" }), milestoneId: uuid("milestone_id").references(() => milestones.id, { onDelete: "set null" }), authorId: uuid("author_id").notNull().references(() => users.id), type: text("type").notNull().default("update"), body: text("body").notNull(), attachmentType: text("attachment_type"), attachmentUrl: text("attachment_url"), attachmentName: text("attachment_name"), status: text("status").notNull().default("visible"), metadata: jsonb("metadata").$type<Record<string, unknown>>().default({}), updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(), deletedAt: timestamp("deleted_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
...
export const timelinePosts = pgTable("timeline_posts", {
  id: uuid("id").defaultRandom().primaryKey(),
  authorId: uuid("author_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  kind: text("kind").$type<"standard" | "birthday">().notNull().default("standard"),
  body: text("body").notNull(),
...
export const birthdayEvents = pgTable("birthday_events", {
  ...
  postId: uuid("post_id").references(() => timelinePosts.id, { onDelete: "set null" }),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
