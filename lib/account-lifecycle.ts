...
export const ACCOUNT_RECOVERY_MS = 30 * 24 * 60 * 60_000;
...
export async function initiateAccountDeactivation(userId: string) {
  ...
  const recoveryDeadline = new Date(now.getTime() + ACCOUNT_RECOVERY_MS);
...
export async function processDueAccountTransitions() {
  ...
  const dueAccounts = await db.select({ id: users.id, email: users.email }).from(users).where(and(eq(users.status, "deactivated"), lte(users.recoveryDeadline, now)));
  for (const account of dueAccounts) await finaliseAccountDeletion(account.id, account.email);
  return { elections, deletedAccounts: dueAccounts.map(account => account.id) };
}

async function finaliseAccountDeletion(userId: string, previousEmail: string) {
  const db = getDb();
  const now = new Date();
