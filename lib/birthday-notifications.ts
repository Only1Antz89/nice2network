...
import { birthdayEvents, notifications, privacySettings, users } from "@/db/schema";
...
export async function sendBirthdayNotifications(now = new Date()) {
  ...
  for (const member of candidates) {
    ...
    const result = await db.transaction(async tx => {
      ...
      if (!event) return { created: false, delivered: 0 };
      ...
      return { created: true, delivered: values.length };
    });
    if (result.created) events += 1;
    delivered += result.delivered;
  }
  return { date: celebrationDate, events, notifications: delivered };
}
