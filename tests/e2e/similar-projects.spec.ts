...
test("pre-publish similarity is default-on, owner-only, role-aware and immediately disableable", async ({ page }, testInfo) => {
  ...
  try {
    ...
    await page.getByRole("button", { name: "Find the gaps" }).click();
    await expect(page.getByText("Review your team map")).toBeVisible();
    await page.getByRole("button", { name: "Approve map & publish" }).click();
